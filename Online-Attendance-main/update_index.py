...
content = re.sub(r'<!-- What will you study in our course -->.*?</section>', '', content, flags=re.DOTALL)

# 2. Remove Carousel / Simplify Attendance section
content = re.sub(r'<!-- Carousel -->\s*<section class="py-5" id="services_new">.*?</section>', '', content, flags=re.DOTALL)

# 3. Replace Developer Section
developer_new = """    <!-- Meet the Developer Section -->
    <section class="py-5 position-relative overflow-hidden" style="background: linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%);">
        ...
        <div class="container-lg px-4 position-relative z-index-1">
            <div class="text-center mb-5" data-aos="fade-up">
                <span class="badge rounded-pill bg-primary px-3 py-2 mb-3" style="background: linear-gradient(135deg, #4481eb 0%, #04befe 100%); color: white; font-weight: 600; letter-spacing: 1px;">THE CREATOR</span>
...
content = re.sub(r'<!-- Know your Instructors -->.*?</section>', developer_new, content, flags=re.DOTALL)

# 4. Replace Footer
footer_new = """    <!-- site-footer -->
...
content = re.sub(r'<!-- site-footer -->.*?</footer>', footer_new, content, flags=re.DOTALL)
